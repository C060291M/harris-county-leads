"""
StackIQ — Somervell County Scraper (Tyler iDS)
Portal: somervellcountytx-web.tylerhost.net
...
log = logging.getLogger("somervell")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://somervellcountytx-web.tylerhost.net/web"
...
async def scrape_somervell(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            try:
                ...
                log.info("Somervell: %d raw items", len(items))
                ...
                log.info("Somervell: page %d found %d distress records", page_num, page_records)
...
async def main_async():
    ...
    log.info("=== Somervell County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_somervell(cutoff, now)
    ...
    payload = {
        ...
        "source": "Somervell County Clerk (Tyler iDS)",
    ...
    with open("dashboard/somervell_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/somervell_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/somervell_records.json")
