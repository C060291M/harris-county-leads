"""
StackIQ — Erath County Scraper (Tyler iDS)
Portal: tyler.co.erath.tx.us
...
log = logging.getLogger("erath")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://tyler.co.erath.tx.us/web"
...
async def scrape_erath(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            try:
                ...
                log.info("Erath: %d raw items", len(items))
                ...
                log.info("Erath: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Erath: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Erath County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_erath(cutoff, now)
    ...
    payload = {
        ...
        "source": "Erath County Clerk (Tyler iDS)",
    ...
    with open("dashboard/erath_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/erath_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/erath_records.json")
