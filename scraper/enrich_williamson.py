...
# Counties confirmed on the same ASP.NET "PublicAccess" platform
# (Harris Govern-style, __VIEWSTATE postback, single #SearchText field)
COUNTIES = {
...
async def search_publicaccess(page, base, last_name):
...
async def enrich_county(browser, cur, conn, county, base, leads):
    ...
    for lead_id, owner in leads:
        try:
            ...
            if len(last) < 3:
                continue
            addr = await search_publicaccess(page, base, last)
            if addr:
                cur.execute("""
        ...
        for county, base in COUNTIES.items():
          try:
            cur.execute("""
                ...
                AND owner NOT ILIKE '%%COUNTY OF%%'
                AND owner NOT ILIKE '%% COUNTY' AND owner NOT ILIKE '%%STATE OF%%'
                AND owner NOT ILIKE '%% ISD%%'
