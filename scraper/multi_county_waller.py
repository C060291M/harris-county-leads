...
log = logging.getLogger("waller")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://wallercountytx-web.tylerhost.net/web"
...
async def scrape_waller(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if accept_btn:
                ...
                log.info("Waller: disclaimer accepted")
        except Exception as e:
            log.warning("Waller: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Waller: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Waller: %d raw items", len(items))
                ...
                log.info("Waller: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Waller: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    all_records = await scrape_waller(cutoff, now)
    ...
    payload = {
        ...
        "source": "Waller County Clerk (Tyler iDS)",
    ...
    with open("dashboard/waller_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/waller_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/waller_records.json")
