"""
StackIQ — Burnet County Scraper (Tyler iDS)
Portal: burnetcountytx-web.tylerhost.net
...
log = logging.getLogger("burnet")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://burnetcountytx-web.tylerhost.net/web"
...
async def scrape_burnet(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Burnet: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Burnet: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Burnet: %d raw items", len(items))
                ...
                log.info("Burnet: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Burnet: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Burnet County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_burnet(cutoff, now)
    ...
    payload = {
        ...
        "source": "Burnet County Clerk (Tyler iDS)",
    ...
    with open("dashboard/burnet_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/burnet_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/burnet_records.json")
