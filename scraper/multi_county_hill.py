...
StackIQ — Hill County Scraper (Tyler iDS)

Portal: hillcountytx-web.tylerhost.net
...
BASE_URL      = "https://hillcountytx-web.tylerhost.net/web"
...
async def scrape_hill(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Hill: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Hill: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Hill: %d raw items", len(items))
                ...
                for item in items:
                    ...
                    records.append({
                        ...
                        "clerk_url": f"{BASE_URL}/search/DOCSEARCH100427S1",
                ...
                log.info("Hill: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Hill: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Hill County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_hill(cutoff, now)
    ...
    payload = {
        ...
        "source": "Hill County Clerk (Tyler iDS)",
    ...
    with open("dashboard/hill_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/hill_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/hill_records.json")
