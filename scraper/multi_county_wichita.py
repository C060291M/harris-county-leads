...
StackIQ — Wichita County Scraper (Tyler iDS)

Portal: wichitacountytx-web.tylerhost.net
...
BASE_URL      = "https://wichitacountytx-web.tylerhost.net/web"
...
async def scrape_wichita(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Wichita: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Wichita: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Wichita: %d raw items", len(items))
                ...
                log.info("Wichita: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Wichita: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Wichita County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_wichita(cutoff, now)
    ...
    payload = {
        ...
        "source": "Wichita County Clerk (Tyler iDS)",
    ...
    with open("dashboard/wichita_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/wichita_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/wichita_records.json")
