...
BASE_URL      = "https://kimblecountytx-web.tylerhost.net/web"
...
async def scrape_kimble(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Kimble: disclaimer error: %s", e)

            log.info("Kimble: disclaimer accepted")
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Kimble: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Kimble: %d raw items", len(items))
                ...
                log.info("Kimble: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Kimble: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    all_records = await scrape_kimble(cutoff, now)
    ...
    payload = {
        ...
        "source": "Kimble County Clerk (Tyler iDS)",
    ...
    with open("dashboard/kimble_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/kimble_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/kimble_records.json")
