"""
StackIQ — Henderson County Scraper (Tyler iDS)
Portal: hendersoncountytx-web.tylerhost.net
...
log = logging.getLogger("henderson")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://hendersoncountytx-web.tylerhost.net/web"
...
async def scrape_henderson(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Henderson: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Henderson: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Henderson: %d raw items", len(items))
                ...
                log.info("Henderson: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Henderson: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Henderson County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_henderson(cutoff, now)
    ...
    payload = {
        ...
        "source": "Henderson County Clerk (Tyler iDS)",
    ...
    with open("dashboard/henderson_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/henderson_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/henderson_records.json")
