...
StackIQ — Mills County Scraper (Tyler iDS)

Portal: millscountytx-web.tylerhost.net
...
BASE_URL      = "https://millscountytx-web.tylerhost.net/web"
...
async def scrape_mills(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Mills: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Mills: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)

            try:

                await page.goto(f"{BASE_URL}/search/DOCSEARCH419S1", wait_until="domcontentloaded", timeout=30000)
                ...
                log.info("Mills: %d raw items", len(items))
                ...
                log.info("Mills: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Mills: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Mills County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_mills(cutoff, now)
    ...
    payload = {
        ...
        "source": "Mills County Clerk (Tyler iDS)",
    ...
    with open("dashboard/mills_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/mills_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/mills_records.json")
