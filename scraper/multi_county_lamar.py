...
StackIQ — Lamar County Scraper (Tyler iDS)

Portal: lamarcountytx-web.tylerhost.net
...
BASE_URL      = "https://lamarcountytx-web.tylerhost.net/web"
...
async def scrape_lamar(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Lamar: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Lamar: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Lamar: %d raw items", len(items))
                ...
                log.info("Lamar: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Lamar: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Lamar County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_lamar(cutoff, now)
    ...
    payload = {
        ...
        "source": "Lamar County Clerk (Tyler iDS)",
    ...
    with open("dashboard/lamar_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/lamar_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/lamar_records.json")
