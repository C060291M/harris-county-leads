...
async def scrape_winkler(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Winkler: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Winkler: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Winkler: %d raw items", len(items))
                ...
                log.info("Winkler: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Winkler: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Winkler County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_winkler(cutoff, now)
    ...
    payload = {
        ...
        "source": "Winkler County Clerk (Tyler iDS)",
    ...
    with open("dashboard/winkler_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/winkler_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/winkler_records.json")
