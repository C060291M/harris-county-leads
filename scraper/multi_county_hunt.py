...
StackIQ — Hunt County Scraper (Tyler iDS)

Portal: huntcountytx-web.tylerhost.net
...
BASE_URL      = "https://huntcountytx-web.tylerhost.net/web"
...
async def scrape_hunt(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Hunt: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Hunt: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Hunt: %d raw items", len(items))
                ...
                log.info("Hunt: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Hunt: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Hunt County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_hunt(cutoff, now)
    ...
    payload = {
        ...
        "source": "Hunt County Clerk (Tyler iDS)",
    ...
    with open("dashboard/hunt_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/hunt_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/hunt_records.json")
