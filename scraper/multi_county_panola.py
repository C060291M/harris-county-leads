...
log = logging.getLogger("panola")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://panolacountytx-web.tylerhost.net/web"
...
async def scrape_panola(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if accept_btn:
                ...
                log.info("Panola: disclaimer accepted")
        except Exception as e:
            log.warning("Panola: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Panola: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Panola: %d raw items", len(items))
                ...
                log.info("Panola: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Panola: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Panola County Scraper (Tyler iDS) (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_panola(cutoff, now)
    ...
    payload = {
        ...
        "source": "Panola County Clerk (Tyler iDS)",
    ...
    with open("dashboard/panola_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/panola_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/panola_records.json")
