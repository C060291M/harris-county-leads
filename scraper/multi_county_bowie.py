...
StackIQ — Bowie County Scraper (Tyler iDS)

Portal: bowiecountytx-web.tylerhost.net
...
BASE_URL      = "https://bowiecountytx-web.tylerhost.net/web"
...
async def scrape_bowie(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Bowie: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Bowie: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)

            try:

                await page.goto(f"{BASE_URL}/search/DOCSEARCH149S1", wait_until="domcontentloaded", timeout=30000)
                ...
                log.info("Bowie: %d raw items", len(items))
                ...
                for item in items:
                    ...
                    records.append({
                        ...
                        "clerk_url": f"{BASE_URL}/search/DOCSEARCH149S1",
                ...
                log.info("Bowie: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Bowie: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Bowie County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_bowie(cutoff, now)
    ...
    payload = {
        ...
        "source": "Bowie County Clerk (Tyler iDS)",
    ...
    with open("dashboard/bowie_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/bowie_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/bowie_records.json")
