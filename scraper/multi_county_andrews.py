"""
StackIQ — Andrews County Scraper (Tyler iDS)
Portal: andrewscountytx-web.tylerhost.net
...
log = logging.getLogger("andrews")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://andrewscountytx-web.tylerhost.net/web"
...
async def scrape_andrews(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Andrews: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Andrews: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Andrews: %d raw items", len(items))
                ...
                log.info("Andrews: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Andrews: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Andrews County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_andrews(cutoff, now)
    ...
    payload = {
        ...
        "source": "Andrews County Clerk (Tyler iDS)",
    ...
    with open("dashboard/andrews_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/andrews_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/andrews_records.json")
