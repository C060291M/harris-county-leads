...
async def search_ector(page, last_name):
    await page.goto(f"{BASE}/search/re_name", timeout=30000, wait_until="domcontentloaded")
    await page.wait_for_timeout(1200)
    lname = await page.query_selector("input[name='lname']")
    if not lname:
        return None
    await lname.fill(last_name)
    submit = await page.query_selector("input[name='findit']")
    await submit.click()
    ...
    for t in tables:
        ...
        try:
            addr_idx = headers.index("Location")
    ...
    async with async_playwright() as p:
        ...
        for lead_id, owner in leads:
            try:
                ...
                if len(last) < 3:
                    continue
                addr = await search_ector(page, last)
                if addr:
                    cur.execute("""
