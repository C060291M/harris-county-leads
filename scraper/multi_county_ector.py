...
BASE_URL      = "https://ectorcountytx-web.tylerhost.net/web"
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if accept_btn:
                ...
                log.info("Ector: disclaimer accepted")
            else:
                log.warning("Ector: I Accept button not found")
        except Exception as e:
            log.warning("Ector: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Ector: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Ector: %d raw items", len(items))
                ...
                log.info("Ector: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Ector: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Ector County Scraper (Tyler iDS) (optimized) ===")
    ...
    with open("dashboard/ector_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/ector_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/ector_records.json")
