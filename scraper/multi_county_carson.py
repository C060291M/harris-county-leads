...
BASE_URL      = "https://carsoncountytx-web.tylerhost.net/web"
...
async def scrape_carson(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if accept_btn:
                ...
                log.info("Carson: disclaimer accepted")
        except Exception as e:
            log.warning("Carson: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Carson: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Carson: %d raw items", len(items))
                ...
                log.info("Carson: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Carson: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    all_records = await scrape_carson(cutoff, now)
    ...
    payload = {
        ...
        "source": "Carson County Clerk (Tyler iDS)",
    ...
    with open("dashboard/carson_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/carson_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/carson_records.json")
