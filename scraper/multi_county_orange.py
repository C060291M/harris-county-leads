...
StackIQ — Orange County Scraper (Tyler iDS)

Portal: orangecountytx-web.tylerhost.net
...
BASE_URL      = "https://orangecountytx-web.tylerhost.net/web"
...
async def scrape_orange(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Orange: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Orange: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Orange: %d raw items", len(items))
                ...
                log.info("Orange: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Orange: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Orange County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_orange(cutoff, now)
    ...
    payload = {
        ...
        "source": "Orange County Clerk (Tyler iDS)",
    ...
    with open("dashboard/orange_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/orange_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/orange_records.json")
