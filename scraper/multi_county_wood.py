"""
StackIQ — Wood County Scraper (Tyler iDS)
Portal: woodcountytx-web.tylerhost.net
...
log = logging.getLogger("wood")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://woodcountytx-web.tylerhost.net/web"
...
async def scrape_wood(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Wood: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Wood: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Wood: %d raw items", len(items))
                ...
                log.info("Wood: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Wood: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Wood County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_wood(cutoff, now)
    ...
    payload = {
        ...
        "source": "Wood County Clerk (Tyler iDS)",
    ...
    with open("dashboard/wood_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/wood_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/wood_records.json")
