...
StackIQ — Liberty County Scraper (Tyler iDS)

Portal: libertycountytx-web.tylerhost.net
...
BASE_URL      = "https://libertycountytx-web.tylerhost.net/web"
...
async def scrape_liberty(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Liberty: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Liberty: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Liberty: %d raw items", len(items))
                ...
                log.info("Liberty: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Liberty: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Liberty County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_liberty(cutoff, now)
    ...
    payload = {
        ...
        "source": "Liberty County Clerk (Tyler iDS)",
    ...
    with open("dashboard/liberty_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/liberty_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/liberty_records.json")
