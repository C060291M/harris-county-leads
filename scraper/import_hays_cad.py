...
owner_map = {}
with open(r"hays_export\PropertyDataExport1391652.txt", encoding="utf-8", errors="replace") as f:
...
segment_map = {}
with open(r"hays_export\PropertyDataExport1391655.txt", encoding="utf-8", errors="replace") as f:
    reader = csv.reader(f, delimiter=",", quotechar='"')
    header = next(reader)
    idx = {name: i for i, name in enumerate(header)}
    for row in reader:
...
batch = []
total = 0
with open(r"hays_export\PropertyDataExport1391651.txt", encoding="utf-8", errors="replace") as f:
    reader = csv.reader(f, delimiter=",", quotechar='"')
    header = next(reader)
    idx = {name: i for i, name in enumerate(header)}
    for row in reader:
        try:
            ...
            if len(batch) >= 5000:
                execute_values(cur, "INSERT INTO hays_cad (property_id, owner_name, situs_address, yr_built, living_area, bedrooms) VALUES %s", batch)
                conn.commit()
                total += len(batch)
...
print(f"DONE: {total} rows imported into hays_cad")
