...
StackIQ — Hardin County Scraper (Tyler iDS)

Portal: hardincountytx-web.tylerhost.net
...
BASE_URL      = "https://hardincountytx-web.tylerhost.net/web"
...
async def scrape_hardin(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Hardin: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Hardin: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Hardin: %d raw items", len(items))
                ...
                log.info("Hardin: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Hardin: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Hardin County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_hardin(cutoff, now)
    ...
    payload = {
        ...
        "source": "Hardin County Clerk (Tyler iDS)",
    ...
    with open("dashboard/hardin_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/hardin_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/hardin_records.json")
