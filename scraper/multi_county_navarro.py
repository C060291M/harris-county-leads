...
StackIQ — Navarro County Scraper (Tyler iDS)

Portal: navarrocountytx-web.tylerhost.net
...
BASE_URL      = "https://navarrocountytx-web.tylerhost.net/web"
...
async def scrape_navarro(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Navarro: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Navarro: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Navarro: %d raw items", len(items))
                ...
                log.info("Navarro: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Navarro: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Navarro County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_navarro(cutoff, now)
    ...
    payload = {
        ...
        "source": "Navarro County Clerk (Tyler iDS)",
    ...
    with open("dashboard/navarro_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/navarro_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/navarro_records.json")
