"""
El Paso CAD Import Script - PACS format
"""
...
INFO_FIELDS = {
    ...
    "prop_type_cd":   (13, 17),
    "owner_name":     (609, 678),
    "situs_prefx":    (1040, 1049),
    "situs_street":   (1050, 1099),
    "situs_suffix":   (1100, 1109),
...
def insert_batch(batch):
    for i in range(5):
        try:
            ...
            cur.executemany("""INSERT INTO elpaso_cad
...
# Find files
info_files = glob.glob("elpaso_cad/*APPRAISAL_INFO.TXT")
imprv_files = glob.glob("elpaso_cad/*APPRAISAL_IMPROVEMENT_DETAIL.TXT")

if not info_files:
    raise Exception("APPRAISAL_INFO.TXT not found - check ls output above")
...
if imprv_files:
    with open(imprv_files[0], "r", encoding="latin-1", errors="ignore") as f:
        for line in f:
            ...
            r = parse_fixed(line, IMPRV_FIELDS)
            pid = r["prop_id"]
            if pid not in imprv:
                try:
                    imprv[pid] = {
                        "yr_built": int(r["yr_built"]) if r["yr_built"].isdigit() else None,
                        "living_area": int(float(r["living_area"])) if r["living_area"] else None,
                    }
                except: pass
...
# Setup table
conn = get_conn(); cur = conn.cursor()
cur.execute("""CREATE TABLE IF NOT EXISTS elpaso_cad (
...
with open(info_files[0], "r", encoding="latin-1", errors="ignore") as f:
    for line in f:
        ...
        batch.append((pid, r["owner_name"], addr, r["situs_city"], r["situs_zip"],
                      appval, imp.get("yr_built"), imp.get("living_area")))
        if len(batch) >= 500:
            insert_batch(batch); total += len(batch)
            logger.info(f"Inserted {total} rows..."); batch = []

if batch:
    insert_batch(batch); total += len(batch)

logger.info(f"Done! Total {total} El Paso properties imported")
