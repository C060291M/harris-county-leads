...
StackIQ — Kaufman County Scraper (Tyler iDS)

Portal: kaufmancountytx-web.tylerhost.net
...
BASE_URL      = "https://kaufmancountytx-web.tylerhost.net/web"
...
async def scrape_kaufman(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Kaufman: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Kaufman: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Kaufman: %d raw items", len(items))
                ...
                log.info("Kaufman: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Kaufman: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Kaufman County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_kaufman(cutoff, now)
    ...
    payload = {
        ...
        "source": "Kaufman County Clerk (Tyler iDS)",
    ...
    with open("dashboard/kaufman_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/kaufman_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/kaufman_records.json")
