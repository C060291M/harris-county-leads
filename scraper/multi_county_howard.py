"""
StackIQ — Howard County Scraper (Tyler iDS)
Portal: howardcountytx-web.tylerhost.net
...
log = logging.getLogger("howard")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://howardcountytx-web.tylerhost.net/web"
...
async def scrape_howard(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Howard: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Howard: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Howard: %d raw items", len(items))
                ...
                log.info("Howard: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Howard: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Howard County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_howard(cutoff, now)
    ...
    payload = {
        ...
        "source": "Howard County Clerk (Tyler iDS)",
    ...
    with open("dashboard/howard_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/howard_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/howard_records.json")
