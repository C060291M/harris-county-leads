...
BASE_URL      = "https://pottercountytx-web.tylerhost.net/web"
...
async def scrape_ector(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Potter: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Potter: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)

            try:

                await page.goto(f"{BASE_URL}/search/DOCSEARCH422S2", wait_until="domcontentloaded", timeout=30000)
                ...
                log.info("Potter: %d raw items", len(items))
                ...
                for item in items:
                    ...
                    records.append({
                        ...
                        "clerk_url": f"{BASE_URL}/search/DOCSEARCH422S2",
                ...
                log.info("Potter: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Potter: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Potter County Scraper (Tyler iDS) (optimized) ===")
    ...
    with open("dashboard/potter_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/potter_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/potter_records.json")
