...
StackIQ — Wise County Scraper (Tyler iDS)

Portal: wisecountytx-web.tylerhost.net
...
BASE_URL      = "https://wisecountytx-web.tylerhost.net/web"
...
async def scrape_wise(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Wise: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Wise: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Wise: %d raw items", len(items))
                ...
                log.info("Wise: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Wise: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Wise County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_wise(cutoff, now)
    ...
    payload = {
        ...
        "source": "Wise County Clerk (Tyler iDS)",
    ...
    with open("dashboard/wise_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/wise_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/wise_records.json")
