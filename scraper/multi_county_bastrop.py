"""
StackIQ — Bastrop County Scraper (Tyler iDS)
Portal: bastroptx-web.tylerhost.net
...
BASE_URL      = "https://bastroptx-web.tylerhost.net/web"
...
async def scrape_bastrop(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Bastrop: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Bastrop: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Bastrop: %d raw items", len(items))
                ...
                log.info("Bastrop: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Bastrop: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Bastrop County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_bastrop(cutoff, now)
    ...
    payload = {
        ...
        "source": "Bastrop County Clerk (Tyler iDS)",
    ...
    with open("dashboard/bastrop_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/bastrop_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/bastrop_records.json")
