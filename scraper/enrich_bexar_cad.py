...
async def search_owner(page, owner_name):
    """Search Bexar CAD by owner name, return list of (address, prop_id)"""
    try:
        ...
        for row in rows[1:]:
            ...
            results.append((addr, prop_id, owner_cell))
...
async def main():
    ...
    # Get Bexar leads missing addresses
    cur.execute("""
    ...
    async with async_playwright() as p:
        ...
        for i, (lead_id, owner, doc_num) in enumerate(leads):
            ...
            if results:
                addr, prop_id, matched_owner = results[0]
                # Validate address looks real
                if (len(addr) > 10 and 
                    any(c.isdigit() for c in addr) and
                    not any(x in addr.lower() for x in ["website", "contact", "phone", "http", "www", "email"])):
                    cur.execute("""
                        UPDATE lead_records 
                        SET prop_address = %s
