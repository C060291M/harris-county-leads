...
COUNTY        = "gregg"
BASE_URL      = "https://aumentumpa.greggcounty.texas.gov"
...
def parse_results(soup):
    """Parse gregg Aumentum results - same structure as Travis"""
    import re
    records = []
...
async def scrape():
    ...
    log.info("[gregg] Scraping %s to %s", start_str, end_str)
    records = []
    ...
    async with async_playwright() as p:
        ...
        if start_inp and end_inp:
            ...
            await page.keyboard.press("Tab")
            await page.wait_for_timeout(300)
            log.info("[gregg] Dates set: %s to %s", start_str, end_str)
        else:
            log.error("[gregg] Date inputs not found")
            await browser.close()
            return records
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("[gregg] Page %d: %d records", page_num, len(page_recs))
    ...
    log.info("[gregg] %d unique records", len(deduped))
    out_dir = os.path.join(os.path.dirname(__file__), "..", "dashboard")
    os.makedirs(out_dir, exist_ok=True)
    with open(os.path.join(out_dir, "gregg_records.json"), "w", encoding="utf-8") as f:
