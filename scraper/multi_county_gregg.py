...
StackIQ — Gregg County Scraper (Tyler iDS)

Portal: greggcountytx-web.tylerhost.net
...
BASE_URL      = "https://greggcountytx-web.tylerhost.net/web"
...
async def scrape_gregg(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Gregg: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Gregg: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Gregg: %d raw items", len(items))
                ...
                log.info("Gregg: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Gregg: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Gregg County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_gregg(cutoff, now)
    ...
    payload = {
        ...
        "source": "Gregg County Clerk (Tyler iDS)",
    ...
    with open("dashboard/gregg_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/gregg_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/gregg_records.json")
