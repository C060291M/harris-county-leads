...
BASE_URL      = "https://taylorcountytx-web.tylerhost.net/web"
...
async def scrape_taylor(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if accept_btn:
                ...
                log.info("Taylor: disclaimer accepted")
        except Exception as e:
            log.warning("Taylor: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Taylor: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Taylor: %d raw items", len(items))
                ...
                log.info("Taylor: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Taylor: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Taylor County Scraper (Tyler iDS) (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_taylor(cutoff, now)
    ...
    payload = {
        ...
        "source": "Taylor County Clerk (Tyler iDS)",
    ...
    with open("dashboard/taylor_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/taylor_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/taylor_records.json")
