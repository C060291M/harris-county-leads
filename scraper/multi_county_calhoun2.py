...
log = logging.getLogger("calhoun2")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://calhouncountytx-web.tylerhost.net/web"
...
async def scrape_calhoun2(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if accept_btn:
                ...
                log.info("Calhoun2: disclaimer accepted")
        except Exception as e:
            log.warning("Calhoun: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Calhoun: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Calhoun: %d raw items", len(items))
                ...
                log.info("Calhoun: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Calhoun: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    all_records = await scrape_calhoun2(cutoff, now)
    ...
    payload = {
        ...
        "source": "Calhoun County Clerk (Tyler iDS)",
    ...
    with open("dashboard/calhoun_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/calhoun_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/calhoun_records.json")
