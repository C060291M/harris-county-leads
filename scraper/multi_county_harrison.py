"""
StackIQ — Harrison County Scraper (Tyler iDS)
Portal: selfservice.harrisoncountytexas.com
...
log = logging.getLogger("harrison")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://selfservice.harrisoncountytexas.com/web"
...
async def scrape_harrison(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Harrison: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Harrison: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Harrison: %d raw items", len(items))
                ...
                log.info("Harrison: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Harrison: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Harrison County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_harrison(cutoff, now)
    ...
    payload = {
        ...
        "source": "Harrison County Clerk (Tyler iDS)",
    ...
    with open("dashboard/harrison_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/harrison_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/harrison_records.json")
