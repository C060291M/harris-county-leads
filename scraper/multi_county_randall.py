...
log = logging.getLogger("randall")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://randallcountytx-web.tylerhost.net/web"
...
async def scrape_randall(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            accept_btn = await page.query_selector("button:has-text('I Accept')")
            if accept_btn:
                await accept_btn.click()
                await page.wait_for_timeout(2000)
                log.info("Randall: disclaimer accepted")
        except Exception as e:
            log.warning("Randall: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Randall: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Randall: %d raw items", len(items))
                ...
                log.info("Randall: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Randall: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Randall County Scraper (Tyler iDS) (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_randall(cutoff, now)
    ...
    payload = {
        ...
        "source": "Randall County Clerk (Tyler iDS)",
    ...
    with open("dashboard/randall_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/randall_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/randall_records.json")
