...
log = logging.getLogger("fortbend_adv")
...
DISTRESS_TYPES = {41:"LISPEN", 1:"AJ", 31:"FEDLIEN", 34:"HEIRSHIP", 37:"JUDGE", 40:"LIEN", 47:"NOTICE", 68:"STLIEN"}
CAT_MAP = {41:("LP","Lis Pendens"),1:("JUD","Abstract of Judgment"),31:("LNFED","Federal Lien"),
           34:("PRO","Heirship"),37:("JUD","Judgment"),40:("LN","Lien"),47:("NOFC","Notice"),68:("LNSTATE","State Lien")}
...
def parse_rows(html, seen):
    ...
    for row in soup.find_all("tr"):
        ...
        grantee = re.sub(r'\s+', ' ', em.group(1)).strip() if em else ""
...
def scrape():
    ...
    with httpx.Client(headers=HEADERS, follow_redirects=True, timeout=20) as client:
        ...
        for doc_idx, val in DISTRESS_TYPES.items():
            ...
            log.info(f"Fort Bend [{doc_idx}] {lbl}: {count} records")
    
    log.info(f"Fort Bend total: {len(all_records)} unique records")
    os.makedirs("dashboard", exist_ok=True)
    with open("dashboard/fortbend_adv_records.json","w") as f:
        json.dump({"county":COUNTY,"total":len(all_records),"records":all_records},f)
    log.info("Saved -> dashboard/fortbend_adv_records.json")
