...
StackIQ — Delta County Scraper (Tyler iDS)

Portal: deltacountytx-web.tylerhost.net
...
BASE_URL      = "https://deltacountytx-web.tylerhost.net/web"
...
async def scrape_delta(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Delta: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Delta: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Delta: %d raw items", len(items))
                ...
                log.info("Delta: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Delta: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Delta County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_delta(cutoff, now)
    ...
    payload = {
        ...
        "source": "Delta County Clerk (Tyler iDS)",
    ...
    with open("dashboard/delta_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/delta_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/delta_records.json")
