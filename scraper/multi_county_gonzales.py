...
StackIQ — Gonzales County Scraper (Tyler iDS)

Portal: gonzalescountytx-web.tylerhost.net
...
log = logging.getLogger("gonzales")
...
BASE_URL      = "https://gonzalescountytx-web.tylerhost.net/web"
...
async def scrape_gonzales(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Gonzales: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Gonzales: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)

            try:

                await page.goto(f"{BASE_URL}/search/DOCSEARCH782S3", wait_until="domcontentloaded", timeout=30000)
                ...
                log.info("Gonzales: %d raw items", len(items))
                ...
                for item in items:
                    ...
                    records.append({
                        ...
                        "clerk_url": f"{BASE_URL}/search/DOCSEARCH782S3",
                ...
                log.info("Gonzales: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Gonzales: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Gonzales County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_gonzales(cutoff, now)
    ...
    payload = {
        ...
        "source": "Gonzales County Clerk (Tyler iDS)",
    ...
    with open("dashboard/gonzales_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/gonzales_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/gonzales_records.json")
