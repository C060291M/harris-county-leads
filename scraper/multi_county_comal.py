...
StackIQ — Comal County Scraper (Tyler iDS)

Portal: comalcountytx-web.tylerhost.net
...
BASE_URL      = "https://comalcountytx-web.tylerhost.net/web"
...
async def scrape_comal(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Comal: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Comal: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Comal: %d raw items", len(items))
                ...
                log.info("Comal: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Comal: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Comal County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_comal(cutoff, now)
    ...
    payload = {
        ...
        "source": "Comal County Clerk (Tyler iDS)",
    ...
    with open("dashboard/comal_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/comal_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/comal_records.json")
