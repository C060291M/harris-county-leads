...
StackIQ — McLennan County Scraper (Tyler iDS)

Portal: mclennancountytx-web.tylerhost.net
...
log = logging.getLogger("mclennan")
...
BASE_URL      = "https://mclennancountytx-web.tylerhost.net/web"
...
async def scrape_mclennan(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("McLennan: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("McLennan: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)

            try:

                await page.goto(f"{BASE_URL}/search/DOCSEARCH402S1", wait_until="domcontentloaded", timeout=30000)
                ...
                log.info("McLennan: %d raw items", len(items))
                ...
                for item in items:
                    ...
                    records.append({
                        ...
                        "clerk_url": f"{BASE_URL}/search/DOCSEARCH402S1",
                ...
                log.info("McLennan: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("McLennan: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== McLennan County Scraper (optimized) ===")
    ...
    payload = {
        ...
        "source": "McLennan County Clerk (Tyler iDS)",
    ...
    with open("dashboard/mclennan_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/mclennan_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    log.info("Saved -> dashboard/mclennan_records.json")
