"""
StackIQ — Yoakum County Scraper (Tyler iDS)
Portal: yoakumcountytx-selfservice.tylerhost.net
...
log = logging.getLogger("yoakum")

LOOKBACK_DAYS = int(os.getenv("LOOKBACK_DAYS", "3"))
MAX_PAGES     = int(os.getenv("MAX_PAGES", "3"))
BASE_URL      = "https://yoakumcountytx-selfservice.tylerhost.net/web"
...
async def scrape_yoakum(start_dt, end_dt):
    records = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Yoakum: disclaimer error: %s", e)

        # Single search by date range only - no doc type filter
        for page_num in range(1, MAX_PAGES + 1):
            log.info("Yoakum: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            try:
                ...
                log.info("Yoakum: %d raw items", len(items))
                ...
                log.info("Yoakum: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Yoakum: page %d error: %s", page_num, e)
                break
...
async def main_async():
    ...
    log.info("=== Yoakum County Scraper (optimized) ===")
    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))

    all_records = await scrape_yoakum(cutoff, now)
    ...
    payload = {
        ...
        "source": "Yoakum County Clerk (Tyler iDS)",
    ...
    with open("dashboard/yoakum_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    with open("data/yoakum_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
    log.info("Saved -> dashboard/yoakum_records.json")
