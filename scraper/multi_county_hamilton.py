...
StackIQ — Hamilton County Scraper (Tyler iDS)
...
async def scrape_hamilton(start_dt, end_dt):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log.warning("Hamilton: disclaimer error: %s", e)
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            log.info("Hamilton: page %d of %d (%s to %s)", page_num, MAX_PAGES, start_str, end_str)
            ...
            try:
                ...
                log.info("Hamilton: %d raw items", len(items))
                ...
                log.info("Hamilton: page %d found %d distress records", page_num, page_records)
                ...
                log.warning("Hamilton: page %d error: %s", page_num, e)
...
async def main_async():
    ...
    log.info("=== Hamilton County Scraper (optimized) ===")

    log.info("Date range: %s to %s", cutoff.strftime("%Y-%m-%d"), now.strftime("%Y-%m-%d"))



    all_records = await scrape_hamilton(cutoff, now)
    ...
    with open("dashboard/hamilton_records.json","w") as f: json.dump(payload,f,indent=2,default=str)

    with open("data/hamilton_records.json","w") as f: json.dump(payload,f,indent=2,default=str)
